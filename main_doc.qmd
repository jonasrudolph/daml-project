...
#### Das Problem mit der Klassifizierung der Jobs nach Technisch und Administrativ lässt sich lösen in dem ich die nicht in 2 tabellen aufteile sondern sie klassifiziere...
...
These Gehaltscap education level vorhanden?
...
3.  **Statistische Tests:** Man könnte statistische Tests durchführen, um festzustellen, ob es signifikante Unterschiede in den Gehältern zwischen den Bildungsniveaus gibt. Zum Beispiel könnten ANOVA oder andere geeignete Tests verwendet werden.

4.  **Regression:** Eine Regressionsanalyse könnte durchgeführt werden, um zu untersuchen, wie das Bildungsniveau das Gehalt beeinflusst. Dies könnte helfen, die Grenzen des Gehaltsniveaus für jedes Bildungsniveau zu verstehen.

5.  **Visualisierungen:** Verschiedene Visualisierungen wie Scatterplots oder Liniendiagramme könnten erstellt werden, um Trends oder Muster zwischen Gehalt und Bildungsniveau zu erkennen.
...
ggplot(average_salary_education, aes(x = factor(Education.Level), y = Salary)) +
  geom_bar(stat = "identity", fill = "skyblue", alpha = 0.7) +
  labs(title = "Durchschnittsgehälter nach Bildungsniveau",
       x = "Bildungsniveau",
       y = "Durchschnittliches Gehalt") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Drehung der x-Achsenbeschriftungen für bessere Lesbarkeit
...
# Annahme: 'Salary' und 'Education.Level' sind die entsprechenden Spalten in Ihrem 'filtered_data' Datensatz

# Scatterplot erstellen
ggplot(filtered_data, aes(x = Education.Level, y = Salary)) +
  geom_point() + 
...
\# Um das Bildungslevel als kontinuierliches Kontinuum zu betrachten

\# Faktorisieren von 'Education.Level' in aufsteigender Reihenfolge

filtered_data\$Education.Level \<- factor(filtered_data\$Education.Level, levels = unique(filtered_data\$Education.Level))
...
# Durchschnittsgehalt für jeden Bildungslevel berechnen
avg_salary_by_education <- filtered_data %>%
  group_by(Education.Level) %>%
  summarise(Avg_Salary = mean(Salary))

# Liniendiagramm erstellen
ggplot(avg_salary_by_education, aes(x = Education.Level, y = Avg_Salary, group = 1)) +
  geom_line() +
  labs(title = "Durchschnittsgehalt nach Bildungslevel",
       x = "Bildungslevel",
...
Anscheinen sehr starker unterschied von 0 auf 1 und 1 auf 2
